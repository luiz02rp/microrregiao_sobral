...
media_indice <- indice_env_sobral$Média
...
Quanto à população predominante na microrregião (jovens, adultos e idosos), observa-se que os adultos estão em maior número (`r média_perc_adultos`). Esse resultado é semelhante à porcentagem para o Brasil (`r média_adultos_brasil`) e para o estado do Ceará (`r média_adultos_ceara`). O índice de envelhecimento (nº de idosos para cada 100 jovens), na microrregião, é de (`r media_indice`). Para o Brasil e o Ceará, o índice é, respectivamente: (`r indice_brasil`) e  e (`r indice_ceara`), o que demonstra que o envelhecimento é levemente inferior ao observado no Brasil como um todo.

Embora o índice de envelhecimento da microrregião seja relativamente alto, alguns municípios apresentam população expressivamente jovem. É o caso de Sobral, município mais populoso da área, que conta com um índice de (`r indice_sobral`). A população jovem, em (`r jovens_sobral`) é quase o dobro da população idosa (`r idosos_sobral`). Isso é relevante pois Sobral é a sede a microrregião e concentra mais da metade dos equipamentos. Uma vez que sua população é majoritariamente jovem, a população idosa, que costuma gerar maior demanda, deve partir das cidades circunvizinhas. Como a tabela demonstra, municípios ao redor ou distantes de Sobral tendem a ter um percentual maior de população idosa.
